...
def run_bench(binary, version, N, filename, nsteps, n_threads, theta=0.5, k=0, extra_env=None):
    ...
    for line in lines:
        ...
        # Hotspot line printed by v5: "[HP] Schedule: X | Tree Build: Xs | Force Compute: Xs"
        if "[HP] Schedule:" in line:
            parts = line.split("|")
            schedule = parts[0].split(":", 1)[1].strip()
            tree_time = float(parts[1].split(":")[1].replace("s", "").strip())
            force_time = float(parts[2].split(":")[1].replace("s", "").strip())
...
def run_comprehensive_suite(binary="./build/nbody_simulate",
    ...
    threads_list = threads_list or [1, 2, 4, 8, 16, 20, 32]
    strong_results = []
