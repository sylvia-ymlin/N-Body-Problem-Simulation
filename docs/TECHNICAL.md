...
```
v1_naive/           # Baseline O(N^2) brute-force (Serial + OpenMP)
v2_barnes_hut/      # O(N log N) with standard malloc
v3_arena/           # Barnes-Hut with custom Arena Allocator
v4_morton/          # Barnes-Hut with Z-order spatial sorting
v5_parallel/        # Final optimized parallel version (K-Means + OpenMP)

scripts/
├── benchmark.py        # Performance comparison of all versions
├── generate_data.py    # Initial condition generator
├── visualize.py        # 2D particle simulation visualizer
└── bench_alloc.c       # Memory allocator micro-benchmark
```
...
### Square-Theta Optimization (Avoid `sqrt`)

In the core force calculation, the theta criterion check `width / sqrt(dist_sq) <= THETA_MAX` was optimized by squaring both sides:

```c
// Optimized check in barnes_hut.c
if (width * width <= THETA_MAX * THETA_MAX * dist_sq) {
    _compute_force(pos_x, pos_y, mass, tNode, fx, fy, G);
}
```

**Impact**: Eliminates millions of `sqrt()` calls per timestep, significantly reducing CPU cycles spent on branch prediction and floating-point math.

### Inverse-Square/Cube Optimization

The force calculation kernel was refactored to use a single `sqrt` and reciprocal multiplications, which are more pipeline-friendly:

```c
// Before: multiple divisions and pow() equivalent
double r_plummer = sqrt(r_squared) + EPSILON_O;
double force_factor = -G * mass * tNode->mass / (r_plummer * r_plummer * r_plummer);

// After: pipeline-friendly math
