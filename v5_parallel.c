...
// v5: Morton ordering with OpenMP parallel force computation.
...
static void compute_force_single(double pos_x, double pos_y, double mass,
  ...
  // Depth stays small for the problem sizes used here, so a fixed stack is
  // simpler than recursive traversal in the hot loop.
  TNode *stack[256];
  int sp = 0;
  if (root)
    stack[sp++] = root;
  ...
  while (sp > 0) {
    ...
    // Accept the Barnes-Hut approximation when the cell is sufficiently far
    // from the target particle; otherwise descend into children.
...
void compute_force_v5_parallel(ParticleSystem *sys, KernelConfig *config) {
  ...
  // Morton codes work best with a square domain; padding also keeps particles
  // away from the exact boundary during insertion.
  x_max = x_min + d;
  y_max = y_min + d;
  x_min -= d * 0.05;
  x_max += d * 0.05;
  ...
  double t_kmeans_start = 0.0;
  double t_kmeans_end = 0.0;
  if (config->k_clusters <= 0) {
    // Default path: Morton ordering.
    z_order_sort(sys, x_min, x_max, y_min, y_max);
  } else {
    // Experimental path kept for ablation studies.
    t_kmeans_start = sim_time_now();
    ...
    t_kmeans_end = sim_time_now();
  }

  // Tree build remains serial; this is the main non-parallel section in v5.
...
#pragma omp parallel for schedule(runtime) num_threads(config->n_threads)
  ...
  printf("\n[HP] Schedule: %s,%d | KMeans: %.4f s | Tree Build: %.4f s | Force Compute: %.4f s\n",
         schedule_name(schedule_kind), schedule_chunk, t_kmeans_end - t_kmeans_start,
