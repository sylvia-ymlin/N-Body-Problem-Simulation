...
/**
 * v1: Brute-Force Baseline - O(N²)
 *
 * Numerical ground truth for accuracy verification.
 * No approximations; direct pairwise force calculation.
 */
void compute_force_v1_naive(ParticleSystem *sys, KernelConfig *config) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    for (int j = 0; j < N; j++) {
      ...
      // G scaled by N for stability; eps prevents singularities
