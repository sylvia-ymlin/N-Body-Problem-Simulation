...
static TNode *create_node(double x_min, double x_max, double y_min, double y_max, NodeArena *arena) {
...
static void insert(TNode *node, int idx, double *px, double *py, double *mass, NodeArena *arena) {
  ...
  int quad = 0;
  if (px[idx] > mid_x) quad += 1;
  if (py[idx] > mid_y) quad += 2;
  ...
  if (!node->children[quad]) {
    ...
    node->children[quad] = create_node(nx_min, nx_max, ny_min, ny_max, arena);
  }
  insert(node->children[quad], idx, px, py, mass, arena);
...
static void compute_force(TNode *node, int idx, double *px, double *py, double *mass,
                          double *fx, double *fy, double THETA_MAX) {
  if (!node || node->mass == 0 || node->particle_idx == idx) return;
  ...
  if (node->is_leaf || (s / r < THETA_MAX)) {
    double G = 100.0 / 1000.0;
    double f = G * mass[idx] * node->mass / (r * r * r);
    *fx += f * dx;
    *fy += f * dy;
  } else {
    for (int i = 0; i < 4; i++) {
      compute_force(node->children[i], idx, px, py, mass, fx, fy, THETA_MAX);
    }
  }
}
...
void barnes_hut(double *px, double *py, double *mass, int N, double *fx,
  ...
  for (int i = 0; i < N; i++) {
    fx[i] = 0; fy[i] = 0;
    compute_force(root, i, px, py, mass, &fx[i], &fy[i], THETA_MAX);
  }
}
