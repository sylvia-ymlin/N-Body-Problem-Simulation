...
static TNode *create_node(double LB, double RB, double DB, double UB, NodeArena *arena) {
  ...
  node->LB = LB; node->RB = RB; node->DB = DB; node->UB = UB;
  node->mass = 0; node->pos_x = 0; node->pos_y = 0;
  node->PID = -1;
...
static void compute_force(TNode *node, int idx, double *px, double *py, double *mass,
  ...
  if (!node || node->mass == 0 || node->PID == idx) return;
  ...
  if (node->PID != -1 || (s / r < THETA_MAX)) {
    ...
    *fy += f * dy;
    ...
    for (int i = 0; i < 4; i++) {
      compute_force(node->child[i], idx, px, py, mass, fx, fy, THETA_MAX);
    }
  }
}
