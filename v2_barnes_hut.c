...
// v2: Barnes-Hut approximation, O(N log N).
...
static void insert(TNode *node, int idx, ParticleSystem *sys) {
  ...
  if (is_leaf(node)) {
    int old = node->PID;
    if (old != -1) {
        // Without this guard, identical coordinates would keep splitting the
        // same cell forever.
...
static void compute_force_rec(TNode *node, int idx, ParticleSystem *sys,
  ...
  // s/r < THETA means the cell is far enough to treat as one aggregate mass.
